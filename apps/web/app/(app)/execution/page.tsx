...
type WorkFilter = 'all' | 'today' | 'upcoming' | 'overdue' | 'blocked' | 'raci_attention' | 'nabh_autogenerated' | 'completed';
type ExecutionMainTab = 'tasks' | 'intake' | 'quality';

export default function ExecutionPage() {
  const { user } = useAuth();
  const canAccessQuality = isQualityCommandAuthorized(user);
  const [mainTab, setMainTab] = useState<ExecutionMainTab>('tasks');
  ...
  const filterCounts = {
    ...
    nabh_autogenerated: workItems.filter(w => w.tags?.includes('AUTOGENERATED') || w.source_type === 'nabh_autogenerated').length,
  ...
  const displayedItems = workItems.filter(item => {
    ...
    if (activeFilter === 'nabh_autogenerated') {
      return item.tags?.includes('AUTOGENERATED') || item.source_type === 'nabh_autogenerated';
    }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 bg-slate-100/90 p-1.5 rounded-2xl border border-slate-200 shadow-xs overflow-x-auto">
        ...
        >
          My Work &amp; Execution Tasks ({workItems.length})
        </button>
        <button
          type="button"
        ...
        >
          Omnichannel Intake Hub (WhatsApp / Email / 1-on-1s)
        </button>
        {canAccessQuality && (
          <button
            type="button"
      ...
      {mainTab === 'intake' && (
        <OmnichannelIntakeHub onTaskCreated={refreshWork} />
      )}

      {mainTab === 'quality' && canAccessQuality && (
        ...
        <div className="flex w-full flex-nowrap items-center gap-1 overflow-x-auto overscroll-x-contain pb-1 sm:w-auto sm:flex-wrap sm:overflow-visible sm:pb-0">
          {(
            [
              ...
              { id: 'nabh_autogenerated', label: 'NABH Autogenerated', count: filterCounts.nabh_autogenerated },
