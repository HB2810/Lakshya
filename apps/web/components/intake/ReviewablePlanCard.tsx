...
export const ReviewablePlanCard: React.FC<ReviewablePlanCardProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border-2 border-indigo-500/30 dark:border-indigo-500/40 rounded-xl p-5 shadow-lg space-y-4">
      {/* Header Banner */}
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          <span className="flex h-2.5 w-2.5 rounded-full bg-indigo-600 animate-pulse"></span>
          <h3 className="text-sm font-bold tracking-wide uppercase text-indigo-600 dark:text-indigo-400">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Main Task Card */}
        <div className="space-y-3 bg-slate-50 dark:bg-slate-950 p-4 rounded-lg border border-slate-200 dark:border-slate-800">
          <div>
            ...
            <input
              ...
              className="w-full rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm font-medium text-slate-900 dark:text-white focus:border-indigo-500 focus:outline-none"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 py-1.5 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:outline-none"
              >
          ...
          {items.length === 0 ? (
            ...
            <div className="space-y-2">
              {items.map((item, index) => (
                <div
                  key={item.client_id}
                  className="flex flex-col md:flex-row md:items-center gap-2 bg-slate-50/70 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200/80 dark:border-slate-800"
                >
                  ...
                  <input
                    ...
                    className="flex-1 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 py-1 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:outline-none"
                  />
                  <select
                    value={item.priority}
                    onChange={(e) =>
                    ...
                    className="w-28 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-1 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:outline-none"
                  >
                    <option value="low">Low</option>
                    <option value="medium">Medium</option>
                    <option value="high">High</option>
                  ...
                  <select
                    ...
                    className="w-36 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-1 text-xs text-slate-900 dark:text-white focus:border-indigo-500 focus:outline-none"
                  >
                  ...
                  <button
                    ...
                    className="text-xs text-red-500 hover:text-red-700 dark:hover:text-red-400 p-1"
