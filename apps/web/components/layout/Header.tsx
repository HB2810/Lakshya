...
import { Bell, Search, LogOut, Clock, Menu, Sparkles } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ onOpenNavigation = () => {} }) => {
  ...
  return (
    <header className="min-h-16 bg-white/85 backdrop-blur-md border-b border-slate-200/80 px-3.5 pt-[env(safe-area-inset-top)] sm:px-5 md:h-16 md:px-6 md:pt-0 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
      ...
      <div className="flex min-w-0 items-center gap-2.5 sm:gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="hidden items-center gap-1.5 text-xs text-slate-500 mt-0.5 sm:flex">
            ...
            <span className="max-w-44 truncate font-semibold text-slate-700">{user.departmentName}</span>
          </div>
        </div>
      </div>

      {/* Top Center: Wisdom of the Day / Kaizen */}
      <div className="hidden xl:flex items-center gap-2 px-3.5 py-1 bg-gradient-to-r from-amber-50/90 to-orange-50/60 border border-amber-200/80 rounded-full text-xs text-amber-900 shadow-2xs">
      ...
      <div className="flex shrink-0 items-center gap-1.5 sm:gap-2.5">
        ...
        <div className="relative hidden xl:block w-52">
          ...
          <input
            type="text"
            placeholder="Search tasks, SOPs..."
            className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-50/80 border border-slate-200/80 rounded-xl text-slate-800 placeholder-slate-400 focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-500/10 transition-all"
          />
        </div>
            ...
            <div className="fixed left-3 right-3 top-[calc(4rem+env(safe-area-inset-top))] mt-2 bg-white/95 backdrop-blur-xl border border-slate-200 rounded-2xl shadow-xl p-4 z-50 animate-in fade-in zoom-in-95 duration-150 sm:absolute sm:left-auto sm:right-0 sm:top-auto sm:w-80">
              ...
              <div className="space-y-2 text-xs">
                <div className="p-3 bg-slate-50/80 rounded-xl border border-slate-100">
                  <p className="font-bold text-slate-900">Welcome to StavyaOne</p>
                  <p className="text-slate-600 text-[11px] mt-0.5">Your hospital workspace and private companion are active.</p>
        ...
        <div className="hidden items-center gap-2 px-3 py-1.5 bg-slate-50/90 border border-slate-200/80 rounded-xl text-xs font-semibold text-slate-700 lg:flex shadow-2xs">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
          <span className="font-bold">{user.name}</span>
          <span className="text-[10px] uppercase font-bold text-slate-600 bg-slate-200/70 px-1.5 py-0.5 rounded-md">
            {user.role}
          </span>
        </div>
        ...
        <button
          ...
          className="flex min-h-10 min-w-10 items-center justify-center text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors cursor-pointer active-press"
          title="Sign out"
          aria-label="Sign out"
        >
