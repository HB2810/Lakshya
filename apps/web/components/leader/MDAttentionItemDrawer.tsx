      ...
      >
        ...
        <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-950/80 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="px-2 py-0.5 text-xs font-bold rounded bg-slate-200/70 dark:bg-slate-800 text-slate-800 dark:text-slate-200 uppercase">
              {item.priority} Priority
            </span>
            {item.is_synthetic && (
              <span className="px-2 py-0.5 text-[10px] font-black rounded bg-amber-500/10 text-amber-800 dark:text-amber-300 border border-amber-500/20 uppercase tracking-wider">
                [Demo / Fallback Data]
              </span>
            )}
        ...
        {conflictError && (
          <div className="p-3 bg-red-50 dark:bg-red-950/40 border-b border-red-200 dark:border-red-800 text-red-800 dark:text-red-300 text-xs font-bold flex items-center justify-between gap-2">
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              ...
              <div className="p-3 rounded-xl bg-purple-50/50 dark:bg-purple-950/20 border border-purple-100 dark:border-purple-900/40 space-y-1">
            ...
            <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-3">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                ...
                <div>
                  ...
                  <span className={`font-black ${item.deferral_count && item.deferral_count > 1 ? 'text-red-600' : 'text-slate-900 dark:text-white'}`}>
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {item.deferral_history.map((def, idx) => (
                      <div key={idx} className="p-2 rounded bg-slate-50 dark:bg-slate-950 text-[11px] space-y-0.5 border border-slate-100 dark:border-slate-800">
                        <div className="flex items-center justify-between text-[10px] text-slate-400 font-medium">
                          <span>{def.author_name}</span>
                          <span>{def.created_at ? new Date(def.created_at).toLocaleDateString() : ''}</span>
                        </div>
                        <p className="text-slate-700 dark:text-slate-300 font-medium">{def.note}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {(item.category === 'HIGH_IMPACT_BLOCKER' || item.status === 'blocked' || item.status === 'stuck') && (
            <div className="p-3.5 rounded-xl bg-rose-50/70 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/50 space-y-2">
              ...
              {item.blocker_details && (
                <div className="grid grid-cols-2 gap-2 pt-1 text-[11px] text-rose-800 dark:text-rose-300">
              ...
              {item.evidence_list && item.evidence_list.length > 0 ? (
                <div className="space-y-2">
                  {item.evidence_list.map((ev, i) => (
                    <div key={i} className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs space-y-1">
                      ...
                      {ev.notes && <p className="text-[11px] text-slate-600 dark:text-slate-400">{ev.notes}</p>}
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                {item.activity_history.map((act) => (
                  <div key={act.id} className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 text-xs space-y-0.5">
                    <div className="flex items-center justify-between text-[10px] text-slate-400">
                      <span className="font-bold text-slate-700 dark:text-slate-300">{act.author_name}</span>
                    ...
                    <div className="text-[11px] text-slate-800 dark:text-slate-200">
                      <span className="font-mono text-[10px] text-indigo-600 dark:text-indigo-400 uppercase font-bold mr-1.5">
                        [{act.activity_type}]
                      </span>
        ...
        <div className="p-4 sm:p-5 border-t border-slate-200 dark:border-slate-800 bg-slate-50/90 dark:bg-slate-950/90 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            ...
            <button
              ...
              className={`p-2.5 text-xs font-bold rounded-xl border flex items-center justify-center gap-1.5 transition-all ${
                isActionAllowed('REQUEST_EVIDENCE')
                  ? 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:border-indigo-400 text-slate-800 dark:text-slate-200 cursor-pointer shadow-2xs'
                  : 'opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-800/40 text-slate-400 border-transparent'
              }`}
            >
            ...
            <button
              ...
              className={`p-2.5 text-xs font-bold rounded-xl border flex items-center justify-center gap-1.5 transition-all ${
                isActionAllowed('VERIFY_EVIDENCE')
                  ? 'bg-white dark:bg-slate-900 border-red-200 dark:border-red-900/60 hover:bg-red-50 dark:hover:bg-red-950/30 text-red-700 dark:text-red-300 cursor-pointer shadow-2xs'
                  : 'opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-800/40 text-slate-400 border-transparent'
              }`}
            >
            ...
            <button
              ...
              className={`p-2.5 text-xs font-bold rounded-xl border flex items-center justify-center gap-1.5 transition-all ${
                isActionAllowed('RECORD_DECISION')
                  ? 'bg-white dark:bg-slate-900 border-purple-200 dark:border-purple-900/60 hover:bg-purple-50 dark:hover:bg-purple-950/30 text-purple-700 dark:text-purple-300 cursor-pointer shadow-2xs'
                  : 'opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-800/40 text-slate-400 border-transparent'
              }`}
            >
            ...
            <button
              ...
              className={`p-2.5 text-xs font-bold rounded-xl border flex items-center justify-center gap-1.5 transition-all ${
                isActionAllowed('EXECUTIVE_OVERRIDE')
                  ? 'bg-white dark:bg-slate-900 border-rose-200 dark:border-rose-900/60 hover:bg-rose-50 dark:hover:bg-rose-950/30 text-rose-700 dark:text-rose-300 cursor-pointer shadow-2xs'
                  : 'opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-800/40 text-slate-400 border-transparent'
              }`}
            >
            ...
            <button
              ...
              className={`p-2.5 text-xs font-bold rounded-xl border flex items-center justify-center gap-1.5 transition-all ${
                isActionAllowed('GRANT_EXTENSION')
                  ? 'bg-white dark:bg-slate-900 border-orange-200 dark:border-orange-900/60 hover:bg-orange-50 dark:hover:bg-orange-950/30 text-orange-700 dark:text-orange-300 cursor-pointer shadow-2xs'
                  : 'opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-800/40 text-slate-400 border-transparent'
              }`}
            >
            ...
            <button
              ...
              className={`p-2.5 text-xs font-bold rounded-xl border flex items-center justify-center gap-1.5 transition-all col-span-2 sm:col-span-3 ${
                isActionAllowed('REASSIGN_RACI')
                  ? 'bg-slate-900 hover:bg-slate-800 text-white dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 border-transparent cursor-pointer shadow-2xs'
                  : 'opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-800/40 text-slate-400 border-transparent'
              }`}
            >
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-150">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 id="modal-action-title" className="text-sm font-black uppercase tracking-wider text-slate-900 dark:text-white">
                ...
                <div className="space-y-3 text-xs">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white"
                    />
                  </div>
                  <div>
                    <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">
                    ...
                    <input
                      ...
                      className="w-24 p-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white"
                    />
                  </div>
                </div>
              )}
              ...
              {activeModal === 'RECORD_DECISION' && (
                <div className="space-y-3 text-xs">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    />
                    <label htmlFor="unblock-check" className="font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
              ...
              {activeModal === 'EXECUTIVE_OVERRIDE' && (
                <div className="space-y-3 text-xs">
                  <div className="p-3 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 rounded-xl text-rose-800 dark:text-rose-300">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="rounded border-slate-300"
                    />
                    <label htmlFor="clear-blocker-check" className="font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
