...
export const NeedsMDAttentionView: React.FC<NeedsMDAttentionViewProps> = ({ onSelectItem }) => {
 ...
 if (isUnauthorized || !isMDOrMDOffice) {
  return (
   <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col items-center justify-center text-center space-y-3 shadow-xs">
 ...
 if (error && !summary) {
  return (
   <div className="p-6 bg-red-50/80 border border-red-200 rounded-2xl space-y-3">
  ...
  <>
   <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm space-y-6">
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
     <div className="space-y-1">
      <div className="flex items-center gap-2 flex-wrap">
       <span className="px-2 py-0.5 text-[10px] font-black uppercase tracking-wider bg-red-500/10 text-red-700 rounded-md border border-red-500/20 flex items-center gap-1">
       ...
       {summary?.is_synthetic_fallback && (
        <span className="px-2 py-0.5 text-[10px] font-black uppercase tracking-wider bg-amber-500/10 text-amber-800 rounded-md border border-amber-500/20 flex items-center gap-1">
     ...
     <div className="flex items-center gap-2">
      <button
       onClick={fetchAttentionItems}
       disabled={isLoading}
       className="p-2 text-slate-400 hover:text-slate-700 :text-slate-200 hover:bg-slate-100 :bg-slate-800 rounded-xl transition-colors cursor-pointer"
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
     {[
      { id: 'ALL', label: 'All Items', count: summary?.total_items || 0, color: 'border-slate-200 text-slate-900 ' },
      { id: 'CRITICAL_OVERDUE', label: 'Critical Overdue', count: summary?.critical_overdue_count || 0, color: 'border-red-200 text-red-700 bg-red-50/50 ' },
      { id: 'HIGH_IMPACT_BLOCKER', label: 'High Blockers', count: summary?.high_impact_blocker_count || 0, color: 'border-rose-200 text-rose-700 bg-rose-50/50 ' },
      { id: 'DECISION_AWAITING_AUTHORITY', label: 'Decisions', count: summary?.decision_awaiting_count || 0, color: 'border-purple-200 text-purple-700 bg-purple-50/50 ' },
      { id: 'EVIDENCE_AWAITING_VERIFICATION', label: 'Verification', count: summary?.evidence_verification_count || 0, color: 'border-amber-200 text-amber-700 bg-amber-50/50 ' },
      { id: 'AT_RISK_MILESTONE', label: 'Milestones', count: summary?.at_risk_milestone_count || 0, color: 'border-blue-200 text-blue-700 bg-blue-50/50 ' },
     ].map(cat => (
      <button
       key={cat.id}
       ...
       className={`p-3 rounded-xl border text-left transition-all cursor-pointer ${
        activeCategory === cat.id
         ...
         : 'hover:border-slate-300 :border-slate-700'
       } ${cat.color}`}
      >
    ...
    <div className="relative">
     ...
     <input
      ...
      className="w-full pl-9 pr-3 py-2 text-xs rounded-xl border border-slate-200 bg-slate-50 text-slate-900 focus:outline-none"
     />
    </div>
    ...
    {filteredItems.length === 0 ? (
     <div className="p-8 text-center bg-slate-50/60 rounded-xl border border-dashed border-slate-200 space-y-2">
      ...
      <h4 className="text-sm font-bold text-slate-900 ">All Executive Items Clear</h4>
      ...
      {filteredItems.map(item => {
       ...
       return (
        <div
         ...
         className="p-4 rounded-xl border border-slate-200 bg-white hover:border-indigo-300 :border-indigo-700 hover:shadow-xs transition-all cursor-pointer space-y-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        >
         ...
         <div className="flex flex-wrap items-center gap-3 text-[11px] text-slate-600 bg-slate-50 p-2.5 rounded-lg border border-slate-100 ">
          <div className="flex items-center gap-1.5">
           <span className="font-bold text-blue-600 ">Responsible (R):</span>
           <span className="font-medium text-slate-900 ">{item.owner_name}</span>
          </div>
          <span className="text-slate-300 ">•</span>
          <div className="flex items-center gap-1.5">
           <span className="font-bold text-purple-600 ">Accountable (A):</span>
           <span className="font-medium text-slate-900 ">{item.accountable_name}</span>
          </div>
         ...
         <div className="p-2.5 rounded-lg bg-amber-50/60 border border-amber-200/70 text-xs text-amber-900 space-y-1">
         ...
         <div className="flex flex-wrap items-center justify-between gap-2 pt-1 text-[10px] text-slate-500 ">
          <div className="flex items-center gap-1.5">
           <span className="font-semibold text-slate-700 ">{item.why_included}</span>
          </div>
