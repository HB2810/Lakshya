...
export const MeetingExecutionBoard: React.FC<MeetingExecutionBoardProps> = ({
  ...
  if (error || !detail) {
    return (
      <div className="p-6 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-xl text-xs text-red-600 dark:text-red-400">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            {onBack && (
              <button onClick={onBack} className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
                ← Back
              </button>
            )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 space-y-4">
          ...
          <form onSubmit={handleAddAgenda} className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-md border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3 py-1.5 text-xs text-slate-900 dark:text-white focus:outline-none"
            />
          ...
          {agenda.length === 0 ? (
            ...
            <div className="space-y-2">
              {agenda.map((item, idx) => (
                <div key={item.id} className="p-3 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-200/70 dark:border-slate-800 flex items-center justify-between">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 space-y-4">
          ...
          <form onSubmit={handleLogDecision} className="space-y-2.5 bg-slate-50 dark:bg-slate-950 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
            <input
              type="text"
              value={newDecisionTitle}
              onChange={(e) => setNewDecisionTitle(e.target.value)}
              placeholder="Decision title..."
              className="w-full rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-1.5 text-xs text-slate-900 dark:text-white focus:outline-none"
              required
            />
            <div className="grid grid-cols-2 gap-2">
              <select
                ...
                className="rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-1 text-xs text-slate-900 dark:text-white"
              >
          ...
          {decisions.length === 0 ? (
            ...
            <div className="space-y-2">
              {decisions.map((dec) => (
                <div key={dec.id} className="p-3 bg-indigo-50/50 dark:bg-indigo-950/30 rounded-lg border border-indigo-100 dark:border-indigo-900/40 flex items-center justify-between">
                  <div>
                    <h5 className="text-xs font-bold text-slate-900 dark:text-white">{dec.title}</h5>
