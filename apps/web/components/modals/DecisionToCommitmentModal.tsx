...
export const DecisionToCommitmentModal: React.FC<DecisionToCommitmentModalProps> = ({
 ...
 return (
  <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
   <div className="bg-white border border-slate-200 rounded-2xl max-w-xl w-full max-h-[calc(100dvh-1.5rem)] overflow-y-auto overscroll-contain p-4 sm:p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150" role="dialog" aria-modal="true" aria-label="Convert decision to commitment">
    {/* Header */}
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
     ...
     <div className="space-y-1">
      ...
      <input
       ...
       required
       className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 bg-slate-50 text-slate-900 focus:outline-none"
      />
     </div>
     ...
     <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
       <div className="space-y-1">
        <label className="block text-[10px] font-bold uppercase tracking-wider text-blue-600 ">
        ...
        <select
         ...
         required
         className="w-full px-2.5 py-1.5 text-xs rounded-lg border border-slate-200 bg-white text-slate-900 "
        >
       ...
       <div className="space-y-1">
        <label className="block text-[10px] font-bold uppercase tracking-wider text-purple-600 ">
       ...
       {tasks.map((task, idx) => (
        <div key={idx} className="flex items-center gap-2">
         <input
          ...
          className="flex-1 px-3 py-1.5 text-xs rounded-lg border border-slate-200 bg-white text-slate-900 "
         />
         <select
          value={task.ownerId}
          onChange={e => handleTaskChange(idx, 'ownerId', e.target.value)}
          className="w-36 px-2 py-1.5 text-xs rounded-lg border border-slate-200 bg-white text-slate-900 "
         >
          {users.map(u => (
