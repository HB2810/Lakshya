    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs">
        <div className="flex items-center gap-2 flex-1">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white focus:outline-none"
            />
          </div>
          <select
            value={filterStatus}
            onChange={e => setFilterStatus(e.target.value)}
            className="px-2.5 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white"
          >
        ...
        <div className="flex items-center gap-2 text-xs">
          <button
            onClick={expandAll}
            className="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 font-medium cursor-pointer"
          >
            Expand All
          </button>
          <button
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          {data.priorities.map(prio => {
            ...
            return (
              <div key={prio.id} className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-slate-50/50 dark:bg-slate-950/40">
                {/* Priority Row */}
                <div
                  onClick={() => toggleNode(prio.id)}
                  className="p-3.5 bg-white dark:bg-slate-900/90 flex items-center justify-between cursor-pointer hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors border-b border-slate-200/80 dark:border-slate-800"
                >
                  <div className="flex items-center gap-3">
                    <button className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                      {isPrioCollapsed ? <ChevronRight className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                    </button>
                    <span className="px-2 py-0.5 text-[10px] font-bold uppercase rounded bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                      2. Priority ({prio.month})
                    </span>
                    <h3 className="text-sm font-bold text-slate-900 dark:text-white">{prio.title}</h3>
                  </div>
                ...
                {!isPrioCollapsed && (
                  <div className="p-4 pl-8 space-y-3">
                    {prio.milestones.map(ms => {
                      ...
                      return (
                        <div key={ms.id} className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900">
                          {/* Milestone Header */}
                          <div
                            onClick={() => toggleNode(ms.id)}
                            className="p-3 flex items-center justify-between cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors border-b border-slate-100 dark:border-slate-800"
                          >
                            <div className="flex items-center gap-2.5">
                              <button className="text-slate-400">
                                {isMsCollapsed ? <ChevronRight className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                              </button>
                              <span className="px-1.5 py-0.5 text-[9px] font-bold uppercase rounded bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
                                3. Milestone {ms.weekCode}
                              </span>
                              <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200">{ms.title}</h4>
                            </div>
                          ...
                          {/* 4. COMMITMENTS */}
                          {!isMsCollapsed && (
                            <div className="p-3 pl-6 space-y-2.5 bg-slate-50/40 dark:bg-slate-950/30">
                              {ms.commitments.map(cm => {
                                const isCmCollapsed = Boolean(collapsedNodes[cm.id]);

                                return (
                                  <div key={cm.id} className="border border-slate-200/80 dark:border-slate-800 rounded-lg bg-white dark:bg-slate-900 overflow-hidden shadow-2xs">
                                    {/* Commitment Header */}
                                    <div
                                      onClick={() => toggleNode(cm.id)}
                                      className="p-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/30"
                                    >
                                      <div className="flex items-center gap-2">
                                        <button className="text-slate-400">
                                          {isCmCollapsed ? <ChevronRight className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                                        </button>
                                        <span className="px-1.5 py-0.5 text-[9px] font-black uppercase rounded bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                                          4. Commitment ({cm.code})
                                        </span>
                                        <h5 className="text-xs font-extrabold text-slate-900 dark:text-white">{cm.title}</h5>
                                      </div>
                                      ...
                                      <div className="flex items-center gap-2 text-[10px]">
                                        <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium">
                                          <strong className="text-blue-600 dark:text-blue-400">R:</strong> {cm.raci.responsible} | <strong className="text-purple-600 dark:text-purple-400">A:</strong> {cm.raci.accountable}
                                        </span>
                                    ...
                                    {/* 5. TASKS */}
                                    {!isCmCollapsed && (
                                      <div className="p-2.5 pl-6 border-t border-slate-100 dark:border-slate-800/60 bg-slate-50/60 dark:bg-slate-950/50 space-y-1.5">
                                        {cm.tasks.map(tk => (
                                          <div
                                            key={tk.id}
                                            onClick={() => onSelectTask?.(tk.id)}
                                            className="p-2 rounded-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between hover:border-blue-400 dark:hover:border-blue-500 cursor-pointer transition-colors"
                                          >
                                            <div className="flex items-center gap-2">
                                              <span className="px-1.5 py-0.2 text-[8px] font-black uppercase rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                                                5. Task
                                              </span>
                                              <span className="text-xs font-medium text-slate-800 dark:text-slate-200">{tk.title}</span>
                                            </div>
